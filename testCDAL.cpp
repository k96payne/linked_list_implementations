...
TEST_CASE( "CASE TWO: iterators", "[iterator]" ) {
	SECTION( "iterating through list (non-const)" ) {
		SECTION( "non-empty list" ) {
			...
			CDAL<int>::iterator listIt = list.begin();
			CDAL<int>::iterator listEnd = list.end();
	...
	SECTION( "iterating through list (const)" ) {
		...
		CDAL<int> const listC = list;
		CDAL<int>::const_iterator listIt = listC.begin();
		CDAL<int>::const_iterator listEnd = listC.end();
	...
	SECTION( "preincrement // postincrement (const)" ) {
		...
		CDAL<int> listC = list;
		CDAL<int>::iterator listIt = listC.begin();
		CDAL<int>::iterator listEnd = listC.end();
	...
	SECTION( "== // != // copy and assignment (non-const)" ) {
		...
		CDAL<int>::iterator copy = listIt;
		CDAL<int>::iterator assignment = list.begin();
		assignment = listIt;
		listIt++;
		REQUIRE( *listIt == 1);
		REQUIRE( *copy == 0 );
		...
		// assigment test two=====================/
		CDAL<int>::iterator A = list.begin();
		CDAL<int>::iterator B = list.begin();
		++B; ++B; ++B;
		A = B;
		REQUIRE( *A == *B );
	}
	...
	SECTION( "== // != // copy and assignment (const)" ) {
		...
		//check for deep copy and assignement======/
		CDAL<int>::const_iterator copy = listIt;
		CDAL<int>::const_iterator assignment = listC.begin();
		assignment = listIt;
		listIt++;
		REQUIRE( *listIt == 1);
		REQUIRE( *copy == 0 );
		...
		// assigment test two=====================/
		CDAL<int>::const_iterator A = listC.begin();
		CDAL<int>::const_iterator B = listC.begin();
		++B; ++B; ++B;
		A = B;
		REQUIRE( *A == *B );
	}
	...
	SECTION( "operator-> (non-const)" ) {
		CDAL<std::vector<std::string>*> vecL{};
		...
		CDAL<std::vector<std::string>*>::iterator it = vecL.begin();
		(*it) -> push_back("one");
		(*it) -> push_back("two");
		++it;
		(*it) -> push_back("three");
		...
		CDAL<std::vector<std::string>*>::iterator read = vecL.begin();
	...
	SECTION( "operator-> (const)" ) {
		...
		CDAL<std::vector<std::string>*> const vecLC = vecL;
		CDAL<std::vector<std::string>*>::const_iterator it = vecLC.begin();
		(*it) -> push_back("one");
		(*it) -> push_back("two");
		++it;
		(*it) -> push_back("three");
		...
		CDAL<std::vector<std::string>*>::const_iterator read = vecLC.begin();
		REQUIRE( (*read) -> at(0) == "one" );
		REQUIRE( (*read)-> at(1) == "two" );
		read++;
		REQUIRE( (*read) -> at(0) == "three" );

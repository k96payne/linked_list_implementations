...
TEST_CASE( "CASE TWO: iterators", "[iterator]" ) {
	...
	SECTION( "iterating through list (const)" ) {
		...
		SDAL<int> const listC = list;
		SDAL<int>::const_iterator listIt = listC.begin();
		SDAL<int>::const_iterator listEnd = listC.end();
	...
	SECTION( "preincrement // postincrement (const)" ) {
		...
		SDAL<int> listC = list;
		SDAL<int>::iterator listIt = listC.begin();
		SDAL<int>::iterator listEnd = listC.end();
	...
	SECTION( "== // != // copy and assignment (non-const)" ) {
		...
		SDAL<int>::iterator copy = listIt;
		SDAL<int>::iterator assignment = list.begin();
		assignment = listIt;
		listIt++;
		REQUIRE( *listIt == 1);
		REQUIRE( *copy == 0 );
		...
		// assigment test two=====================/
		SDAL<int>::iterator A = list.begin();
		SDAL<int>::iterator B = list.begin();
		++B; ++B; ++B;
		A = B;
		REQUIRE( *A == *B );
	}
	...
	SECTION( "== // != // copy and assignment (const)" ) {
		...
		//check for deep copy and assignement======/
		SDAL<int>::const_iterator copy = listIt;
		SDAL<int>::const_iterator assignment = listC.begin();
		assignment = listIt;
		listIt++;
		REQUIRE( *listIt == 1);
		REQUIRE( *copy == 0 );
		...
		// assigment test two=====================/
		SDAL<int>::const_iterator A = listC.begin();
		SDAL<int>::const_iterator B = listC.begin();
		++B; ++B; ++B;
		A = B;
		REQUIRE( *A == *B );
	}
	...
	SECTION( "operator-> (non-const)" ) {
		SDAL<std::vector<std::string>*> vecL{};
		...
		SDAL<std::vector<std::string>*>::iterator it = vecL.begin();
		(*it) -> push_back("one");
		(*it) -> push_back("two");
		++it;
		(*it) -> push_back("three");
		...
		SDAL<std::vector<std::string>*>::iterator read = vecL.begin();
	...
	SECTION( "operator-> (const)" ) {
		...
		SDAL<std::vector<std::string>*> const vecLC = vecL;
		SDAL<std::vector<std::string>*>::const_iterator it = vecLC.begin();
		(*it) -> push_back("one");
		(*it) -> push_back("two");
		++it;
		(*it) -> push_back("three");
		...
		SDAL<std::vector<std::string>*>::const_iterator read = vecLC.begin();
		REQUIRE( (*read) -> at(0) == "one" );
		REQUIRE( (*read)-> at(1) == "two" );
		read++;
		REQUIRE( (*read) -> at(0) == "three" );

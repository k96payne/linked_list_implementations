//===============================================//
// Simple Singly Linked List Template Class      //
// Kyle Payne                                    //
// Project 1                                     //
//===============================================//

#ifndef SSLL_H_
#define SSLL_H_

#include <iostream>
#include <stdexcept>

#include "List.h"

namespace cop3530 {

template < typename E >
class SSLL : public List< E > {
	public:
		// Iterator
		//--------------------------------------------------------//
		template< typename SsllT, typename NodeT, typename T >
		class Ssll_Iter {
			public:
				// Required Type Aliases
				//========================================================//
				using value_type = T;
 				using reference = T&;
 				using pointer = T*;
				using difference_type = std::ptrdiff_t;
				using iterator_category = std::forward_iterator_tag;
				//========================================================//

				// Additional Type Aliases
				//========================================================//
				using self_type = Ssll_Iter;
				using self_reference = Ssll_Iter&;
				//========================================================//



				// Factories
				//========================================================//
				static self_type create_begin( SsllT& obj ) { 
					Ssll_Iter s( obj, obj.head ); 
					return s; 
				}
				static self_type create_end( SsllT& obj ) { 
					if( obj.head != nullptr ) {
						Ssll_Iter s( obj, obj.tail -> nextNode ); 
						return s;
					}
					else {
						Ssll_Iter s( obj, obj.head ); 
						return s;
					}
				}
				//========================================================//



				// Iterator Copy and Assignment
				//========================================================//
				self_reference operator=( Ssll_Iter const& s ) {
					if( &s == this ) { return *this; }
					curr = s.curr;
					return *this;
				}
				//========================================================//



				// Iterator Operations
				//========================================================//
				reference operator*() const {
					if( obj.head == nullptr )
						throw std::runtime_error("ERROR using reference : Iterator Exhausted: no such point");
					else if( curr == obj.tail -> nextNode )
						throw std::runtime_error("ERROR using reference : Iterator Exhausted: no such point");

					return curr -> data;
				}

				pointer operator->() const { return &(operator*()); }

				bool operator==( Ssll_Iter const& rhs ) const {
					return (&obj == &rhs.obj && curr == rhs.curr);
				}

				bool operator!=( Ssll_Iter const& rhs ) const {
					return (&obj != &rhs.obj || curr != rhs.curr);
				}

				self_reference operator++() {
					if( obj.head == nullptr )
						throw std::runtime_error("ERROR using ++ : Iterator Exhausted: no such point");
					else if( curr == obj.tail -> nextNode )
						throw std::runtime_error("ERROR using ++ : Iterator Exhausted: no such point");

					curr = curr -> nextNode;
					return *this;

				}

				self_type operator++(int) {
					if( obj.head == nullptr )
						throw std::runtime_error("ERROR using ++(int) : Iterator Exhausted: no such point");
					else if( curr == obj.tail -> nextNode )
						throw std::runtime_error("ERROR using ++(int) : Iterator Exhausted: no such point");

					self_type tmp(*this);
					++( *this );
					return tmp;
				}
				//========================================================//


			private:
				SsllT& obj;
				NodeT * curr;

				// Iterator Constructor
				//--------------------------------------------------------//
				Ssll_Iter( SsllT& s, NodeT * start) : obj( s ), curr( start ) { }

		}; //close iterator class

	public:
		// Constructor
		SSLL( void );

		// Copy Constructor
		SSLL( const SSLL& source );

		// Overload Assignment
		SSLL< E >& operator=( const SSLL& source );

		// Move Constuctor
		SSLL( SSLL&& source );

		// Overload Move Assignment
		SSLL< E >& operator=( SSLL&& source );


		// Operations
		//========================================================//
		void insert( E element, size_t position ) override ; //Insert element as specified index
		void push_back( E element ) override ; //Add element to back of list
		void push_front( E element ) override ; //Add element to front of list
		E replace( E element, size_t position ) override ; //Replace and return element at specified index
		E remove( size_t position ) override ; //Remove and return element at specified index
		E pop_back( void ) override ; //Remove and return element from end of list
		E pop_front( void ) override ; //Remove return element from front of list
		E& item_at( size_t position ) override ; //Return element at specified Index
		E& peek_back( void ) override ; //Return element at back of list
		E& peek_front( void ) override ; //Return element at fron of list
		bool is_empty( void ) override ; //Return true if list is empty
		bool is_full( void ) override ; //Return true if list is full
		size_t length( void ) override ; //Return length of list
		void clear( void ) override ; //Clear list
		bool contains( E element, bool(*fnct)(const E&, const E&) ) override ; //Return true if list contains specified element
		std::ostream& print( std::ostream& os ) override ; //Print list
		E * contents( void ) override ; //Return array of list contents
		//========================================================//


		// Destructor
		~SSLL( void ) override ;

	private:
		struct Node { E data; Node * nextNode = nullptr; };

		Node * head, * tail;

	public:
		// Iterator Creators
		//========================================================//
		//using size_t = std::size_t;
		using value_type = E;
		using iterator = Ssll_Iter<SSLL, Node, value_type>;
		using const_iterator = Ssll_Iter< SSLL const, Node const, value_type const>;
		const_iterator begin( void ) const { return const_iterator::create_begin( *this ); }
		const_iterator end( void ) const { return const_iterator::create_end( *this ); }
		iterator begin( void ) { return iterator::create_begin( *this ); }
		iterator end( void ) { return iterator::create_end( *this ); }
		//========================================================//

}; //close ssll class


// CONSTRUCTOR
//========================================================//
template< typename E >
SSLL< E >::SSLL( void ) {
	head = nullptr;
	tail = nullptr;
	head = tail;
}
//========================================================//



// COPY CONSTRUCTOR
//========================================================//
template< typename E >
SSLL< E >::SSLL( const SSLL& source ) {
	Node * next = nullptr;
	Node * curr = nullptr;

	if( source.head == nullptr ) {
		this -> head = nullptr;
		this -> tail = nullptr;
		head = tail;
		return;
	}
	else {
		this -> head = new Node;
		this -> head -> data = source.head -> data;

		curr = this -> head;
		next = source.head -> nextNode;
	}

	while( next != nullptr ) {
		curr -> nextNode = new Node;
		curr = curr -> nextNode;
		curr -> data = next -> data;
		next = next -> nextNode;
	}

	curr -> nextNode = nullptr;
	tail = curr;
} 
//========================================================//



// OVERLOAD ASSIGNMENT
//========================================================//
template< typename E >
SSLL< E >& SSLL< E >::operator=( const SSLL& source ) {
	//Check for self assignment
	if( this == &source ) return *this;

	// Delete Data
	//------------------------------------//
	if( head != tail ) {
		while( head != tail ) {
			Node * next = head -> nextNode;
			delete head;
			head = next;
		}
	}

	head = nullptr;
	tail = nullptr;
	head = tail;
	//------------------------------------//

	Node * next = nullptr;
	Node * curr = nullptr;

	if( source.head == nullptr ) {
		this -> head = nullptr;
		this -> tail = nullptr;
		head = tail;
		return *this;
	}
	else {
		this -> head = new Node;
		this -> head -> data = source.head -> data;

		curr = this -> head;
		next = source.head -> nextNode;
	}

	while( next != nullptr ) {
		curr -> nextNode = new Node;
		curr = curr -> nextNode;
		curr -> data = next -> data;
		next = next -> nextNode;
	}

	curr -> nextNode = nullptr;
	tail = curr;

	return *this;
}
//========================================================//



// MOVE CONSTRUCTOR
//========================================================//
template< typename E >
SSLL< E >::SSLL( SSLL&& source ) {
	head = source.head;
	tail = source.tail;

	source.head = nullptr;
	source.tail = nullptr;
	source.head = source.tail;
}
//========================================================//



// OVERLOAD MOVE ASSIGNMENT
//========================================================//
template< typename E >
SSLL< E >& SSLL< E >::operator=( SSLL&& source ) {
	//Check for self assignment
	if( this == &source ) return *this;

	// Delete Data
	//------------------------------------//
	while( head != tail ) {
			Node * next = head -> nextNode;
			delete head;
			head = next;
		}
	delete head;
	head = nullptr;
	tail = nullptr;
	head = tail;
	//------------------------------------//

	head = source.head;
	tail = source.tail;

	source.head = nullptr;
	source.tail = nullptr;
	source.head = source.tail;

	return *this;
}
//========================================================//



// DESTRUCTOR
//========================================================//
template< typename E >
SSLL< E >::~SSLL( void ) {
	while( head != tail ) {
		Node * next = head -> nextNode;
		delete head;
		head = next;
	}

	delete head;
	head = nullptr;
	tail = nullptr;
	head = tail;
}
//========================================================//



// insert
//========================================================//
template< typename E >
void SSLL< E >::insert( E element, size_t position ) {
	if( position > length() ) 
		throw std::runtime_error("SSLL< E >::insert() ERROR: Index out of Bounds");

	else if( position == 0 ) push_front( element );
	else if( position == length() ) push_back( element );

	else {
		Node * insert = new Node;
		insert -> data = element;

		Node * previous;
		Node * current = head;

		for( size_t i = 0; i < position; i++ ) {
			previous = current;
			current = current -> nextNode;
		}

		previous -> nextNode = insert;
		insert -> nextNode = current;
	}
}
//========================================================//



// push_back
//========================================================//
template< typename E >
void SSLL< E >::push_back( E element ) {
	Node * insert = new Node;
	insert -> data = element;
	insert -> nextNode = nullptr;

	if( head != nullptr ) {
		tail -> nextNode = insert;
		tail = insert;
		insert = nullptr;
	}
	else {
		head = insert;
		tail = insert;
	}
}
//========================================================//



// push_front
//========================================================//
template< typename E >
void SSLL< E >::push_front( E element ) {
	Node * insert = new Node;
	insert -> data = element;
	if( head != nullptr) {
		insert -> nextNode = head;
		head = insert;
	}
	else {
		head = insert;
		tail = insert;
		tail -> nextNode = nullptr;
	}
}
//========================================================//



// replace
//========================================================//
template< typename E >
E SSLL< E >::replace( E element, size_t position ) {
	if( head == nullptr )
		throw std::runtime_error("SSLL< E >::replace() ERROR: List Empty");
	else if( position >= length() )
		throw std::runtime_error("SSLL< E >::replace() ERROR: Index out of Bounds");

	Node * current = head;
	for ( size_t i = 0; i < position; i ++ ) {
		current = current -> nextNode;
	}
	E replaced = current -> data;

	current -> data = element;

	return replaced;
}
//========================================================//



// remove
//========================================================//
template< typename E >
E SSLL< E >::remove( size_t position ) {
	if( head == nullptr )
		throw std::runtime_error("SSLL< E >::remove() ERROR: List Empty");
	else if(position >= length())
		throw std::runtime_error("SSLL< E >::remove() ERROR: Index out of Bounds");
	
	if(position == 0) { return pop_front(); }
	if(position == length() - 1) {return pop_back(); }

	Node * deletion, * current;
	deletion = head;

	for ( size_t i = 0; i < position; i ++ ) {
		current = deletion;
		deletion = deletion -> nextNode;
	}

	current -> nextNode = deletion -> nextNode;
	E element = deletion -> data;

	delete deletion;

	return element;
}
//========================================================//



// pop_back
//========================================================//
template< typename E >
E SSLL< E >::pop_back( void ) {
	if( head == nullptr )
		throw std::runtime_error("SSLL< E >::pop_back() ERROR: List Empty");

	if( length() == 1 ) return pop_front();

	Node * current, * deletion;
	deletion = head;

	while( deletion -> nextNode != nullptr ) {
		current = deletion;
		deletion = deletion -> nextNode;
	}

	current -> nextNode = nullptr;
	tail = current;
	E element = deletion -> data;
	current -> nextNode = nullptr;

	delete deletion;

	return element;
}
//========================================================//



// pop_front
//========================================================//
template< typename E >
E SSLL< E >::pop_front( void ) {
	if( head == nullptr )
		throw std::runtime_error("SSLL< E >::pop_front() ERROR: List Empty");

	E element;

	if( length() != 1 ) {
		Node * deletion = head;
		head = deletion -> nextNode;
		element = deletion -> data;
		delete deletion;
	}
	else {
		element = tail -> data;
		delete tail;
		head = nullptr;
		tail = nullptr;
		head = tail;
	}
	
	return element;
}
//========================================================//



// item_at
//========================================================//
template< typename E >
E& SSLL< E >::item_at( size_t position) {
	if( head == nullptr )
		throw std::runtime_error("SSLL< E >::item_at() ERROR: List Empty");
	else if( position >= length() )
		throw std::runtime_error("SSLL< E >::item_at() ERROR: Index out of Bounds");

	Node * inspect = head;

	for ( size_t i = 0; i < position; i ++ )
		inspect = inspect -> nextNode;

	return inspect -> data;
}
//========================================================//



// peek_back
//========================================================//
template< typename E >
E& SSLL< E >::peek_back( void ) {
	if( head == nullptr )
		throw std::runtime_error("SSLL< E >::peek_back() ERROR: List Empty");

	return tail -> data; 
}
//========================================================//



// peek_front
//========================================================//
template< typename E >
E& SSLL< E >::peek_front( void ) {
	if( head == nullptr )
		throw std::runtime_error("SSLL< E >::peek_front() ERROR: List Empty");

	return head -> data;
}
//========================================================//



// is_empty
//========================================================//
template< typename E >
bool SSLL< E >::is_empty( void ) { return head == nullptr; }
//========================================================//



// is_full
//========================================================//
template< typename E >
bool SSLL< E >::is_full( void ) { return false; }
//========================================================//



// length
//========================================================//
template< typename E >
size_t SSLL< E >::length( void ) {
	Node * current = head;
	size_t length = 0;

	while( current != nullptr ) {
		current = current -> nextNode;
		length++;
	}

	return length;
}
//========================================================//



// clear
//========================================================//
template< typename E >
void SSLL< E >::clear( void ) {
	while( head != tail ) {
		Node * next = head -> nextNode;
		delete head;
		head = next;
	}

	delete head;

	head = nullptr;
	tail = nullptr;
	head = tail;
}
//========================================================//



// contains
//========================================================//
template< typename E >
bool SSLL< E >::contains( E element, bool (*fctn)(const E&, const E&) ) {
	Node * current = head;
	while ( current != nullptr ) {
		if ( fctn(current -> data, element) == true )
			return true;

		current = current -> nextNode;
	}

	return false;
}
//========================================================//



// print
//========================================================//
template< typename E >
std::ostream& SSLL< E >::print( std::ostream& os ) {
	if( head == nullptr )
		os << "<empty list>" << std::endl;
	else {
		Node * disp = head;

		os << "[";

		while ( disp != nullptr ) {
			os << disp -> data;
			disp = disp -> nextNode;
				if( disp != nullptr ) 
					os << ",";
		}

		os << "]" << std::endl;
	}

	return os;
}
//========================================================//



// contents
//========================================================//
template< typename E >
E * SSLL< E >::contents( void ) {
	E * arr = new E[ length() ];
	Node * current = head;

	int index = 0;

	while( current != nullptr ) {
		arr[index] = current -> data;
		current = current -> nextNode;
		index++;
	}

	return arr;
}
//========================================================//

} //close namespace

#endif

...
TEST_CASE( "CASE TWO: iterators", "[iterator]" ) {
	...
	SECTION( "iterating through list (const)" ) {
		...
		SSLL<int> const listC = list;
		SSLL<int>::const_iterator listIt = listC.begin();
		SSLL<int>::const_iterator listEnd = listC.end();
	...
	SECTION( "preincrement // postincrement (const)" ) {
		...
		SSLL<int> listC = list;
		SSLL<int>::iterator listIt = listC.begin();
		SSLL<int>::iterator listEnd = listC.end();
	...
	SECTION( "== // != // copy and assignment (non-const)" ) {
		...
		SSLL<int>::iterator copy = listIt;
		SSLL<int>::iterator assignment = list.begin();
		assignment = listIt;
		listIt++;
		REQUIRE( *listIt == 1);
		REQUIRE( *copy == 0 );
		...
		// assigment test two=====================/
		SSLL<int>::iterator A = list.begin();
		SSLL<int>::iterator B = list.begin();
		++B; ++B; ++B;
		A = B;
		REQUIRE( *A == *B );
	}
	...
	SECTION( "== // != // copy and assignment (const)" ) {
		...
		//check for deep copy and assignement======/
		SSLL<int>::const_iterator copy = listIt;
		SSLL<int>::const_iterator assignment = listC.begin();
		assignment = listIt;
		listIt++;
		REQUIRE( *listIt == 1);
		REQUIRE( *copy == 0 );
		...
		// assigment test two=====================/
		SSLL<int>::const_iterator A = listC.begin();
		SSLL<int>::const_iterator B = listC.begin();
		++B; ++B; ++B;
		A = B;
		REQUIRE( *A == *B );
	}
	...
	SECTION( "operator-> (non-const)" ) {
		SSLL<std::vector<std::string>*> vecL{};
		...
		SSLL<std::vector<std::string>*>::iterator it = vecL.begin();
		(*it) -> push_back("one");
		(*it) -> push_back("two");
		++it;
		(*it) -> push_back("three");
		...
		SSLL<std::vector<std::string>*>::iterator read = vecL.begin();
	...
	SECTION( "operator-> (const)" ) {
		...
		SSLL<std::vector<std::string>*> const vecLC = vecL;
		SSLL<std::vector<std::string>*>::const_iterator it = vecLC.begin();
		(*it) -> push_back("one");
		(*it) -> push_back("two");
		++it;
		(*it) -> push_back("three");
		...
		SSLL<std::vector<std::string>*>::const_iterator read = vecLC.begin();
		REQUIRE( (*read) -> at(0) == "one" );
		REQUIRE( (*read)-> at(1) == "two" );
		read++;
		REQUIRE( (*read) -> at(0) == "three" );

...
TEST_CASE( "CASE TWO: iterators", "[iterator]" ) {
	...
	SECTION( "iterating through list (const)" ) {
		...
		PSLL<int> const listC = list;
		PSLL<int>::const_iterator listIt = listC.begin();
		PSLL<int>::const_iterator listEnd = listC.end();
	...
	SECTION( "preincrement // postincrement (const)" ) {
		...
		PSLL<int> listC = list;
		PSLL<int>::iterator listIt = listC.begin();
		PSLL<int>::iterator listEnd = listC.end();
	...
	SECTION( "== // != // copy and assignment (non-const)" ) {
		...
		PSLL<int>::iterator copy = listIt;
		PSLL<int>::iterator assignment = list.begin();
		assignment = listIt;
		listIt++;
		REQUIRE( *listIt == 1);
		REQUIRE( *copy == 0 );
		...
		// assigment test two=====================/
		PSLL<int>::iterator A = list.begin();
		PSLL<int>::iterator B = list.begin();
		++B; ++B; ++B;
		A = B;
		REQUIRE( *A == *B );
	}
	...
	SECTION( "== // != // copy and assignment (const)" ) {
		...
		//check for deep copy and assignement======/
		PSLL<int>::const_iterator copy = listIt;
		PSLL<int>::const_iterator assignment = listC.begin();
		assignment = listIt;
		listIt++;
		REQUIRE( *listIt == 1);
		REQUIRE( *copy == 0 );
		...
		// assigment test two=====================/
		PSLL<int>::const_iterator A = listC.begin();
		PSLL<int>::const_iterator B = listC.begin();
		++B; ++B; ++B;
		A = B;
		REQUIRE( *A == *B );
	}
	...
	SECTION( "operator-> (non-const)" ) {
		PSLL<std::vector<std::string>*> vecL{};
		...
		PSLL<std::vector<std::string>*>::iterator it = vecL.begin();
		(*it) -> push_back("one");
		(*it) -> push_back("two");
		++it;
		(*it) -> push_back("three");
		...
		PSLL<std::vector<std::string>*>::iterator read = vecL.begin();
	...
	SECTION( "operator-> (const)" ) {
		...
		PSLL<std::vector<std::string>*> const vecLC = vecL;
		PSLL<std::vector<std::string>*>::const_iterator it = vecLC.begin();
		(*it) -> push_back("one");
		(*it) -> push_back("two");
		++it;
		(*it) -> push_back("three");
		...
		PSLL<std::vector<std::string>*>::const_iterator read = vecLC.begin();
		REQUIRE( (*read) -> at(0) == "one" );
		REQUIRE( (*read)-> at(1) == "two" );
		read++;
		REQUIRE( (*read) -> at(0) == "three" );

...
int main() {
	...
	CBL<int> const Clist = list;

	CBL<int>::const_iterator ClistIt = Clist.begin();
	CBL<int>::const_iterator ClistEnd = Clist.end();

	for( ; ClistIt != ClistEnd; ++ClistIt )  { }


	// rule of five ===================//
	CBL<int> listCopy = list;
	
	CBL<int> listAssignment{};

	listAssignment = list;

	CBL<int> listCM = std::move(list);
